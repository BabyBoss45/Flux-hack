...
export function DesignBriefWizard({
  ...
  const confirmMultiSelect = useCallback(async (messageId: string) => {
    ...
    // Show completion message with Approve button
    await addBotMessage("Perfect! Here's your design brief:\n\n✓ Ready to generate designs", {
      type: 'grid',
      items: [{ label: 'Approve & Continue', value: 'approve', icon: '✓' }],
      columns: 2,
    });
  }, [messages, customInput, addUserMessage, addBotMessage, saveToProject]);
